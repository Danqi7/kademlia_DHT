...
func main() {
	...
	PingMsg := new(libkademlia.PingMessage)
	PingMsg.Sender = kadem.SelfContact
	PingMsg.MsgID = libkademlia.NewRandomID()

	//Dial RPC
	var PongMsg libkademlia.PongMessage
	...
	if PongMsg.MsgID.Equals(PingMsg.MsgID) {
		...
		kadem.UpdateContact(&update)
	}

	log.Printf("ping msgID: %s\n", PingMsg.MsgID.AsString())
	log.Printf("pong msgID: %s\n\n", PongMsg.MsgID.AsString())
...
func executeLine(k *libkademlia.Kademlia, line string) (response string) {
	...
	case toks[0] == "vanish":
		if len(toks) < 5 {
			response = "usage: vanish [VDO ID] [data] [numberKeys] [threshold] ([timeout seconds])"
			return
		}
		...
		// timeoutSeconds := 12 // timeoutSeconds is 60 seconds
		var timeoutSeconds int
		if len(toks) == 6 {
			timeoutSeconds, err = strconv.Atoi(toks[5])
			if err != nil {
				response = fmt.Sprintf("Error: vanish timeout passed is too large, not an int")
				return
			}
		} else {
			response = fmt.Sprintf("Default timeout of 12 seconds is used\n")
			timeoutSeconds = 12
		}
